...
**`theplugai.live` is serving a stale 2026-07-05 build.** The Vercel project `agent-world` is git-linked to the archived `studious-umbrella` repo, not `mundi-agent-dashboard` — confirmed via `list_deployments`, which shows no deployment sourced from a `mundi-agent-dashboard` commit after `b0dbf47` (2026-07-05). Two PRs merged to `mundi-agent-dashboard` main tonight (2026-08-30, #7 links-panel fix and #8 CORS fix) will not appear live until the Git connection is repointed in Vercel project settings. Full detail in [[09-SYSTEM/Command Center|Command Center]].
