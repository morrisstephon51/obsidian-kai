---
type: system-map
status: reference
created: 2026-06-13
---

# 🗺️ Core Relationships Map

**Visual guide to how everything connects in your vault.**

---

## 🎯 The Big Picture

```
YOUR VALUES ([[Core Foundation]])
    ↓
YOUR LIFE VISION (Where you're going)
    ↓
YOUR GOALS (Career, Finance, Education, Development)
    ↓
YOUR PROJECTS (The Plug AI, etc.)
    ↓
YOUR DAILY WORK (Master Tracker, Daily Notes)
    ↓
WEEKLY INSIGHTS (Reviews aggregate patterns)
    ↓
FEEDBACK LOOPS (Update understanding of goals/vision)
```

---

## 💎 Foundation Layer

**[[../03-NOTES/Identity/me|Personal Profile]]** (Master reference)
- Your identity, patterns, motivations
- Informs all decisions
- Updated quarterly

**[[../Areas/Personal Growth/index|Personal Growth]]**
- [[../02-PROJECTS/Personal Growth/Core Values|Core Values]] ← START HERE
- [[../02-PROJECTS/Personal Growth/Life Vision|Life Vision]] ← North Star
- [[../02-PROJECTS/Personal Growth/Strengths|Strengths]] ← Your assets
- [[../02-PROJECTS/Personal Growth/Weaknesses|Weaknesses]] ← Areas to watch
- [[../02-PROJECTS/Personal Growth/Motivations|Motivations]] ← What drives you

---

## 🎯 Goals Layer

**[[../Areas/Career/index|Career]]** ← Enabled by values + education
- Goal: [[../02-PROJECTS/Career/Career Goals|Career Goals]]
- Support: [[../02-PROJECTS/Development/Skills|Skill development]]
- Measured by: [[../02-PROJECTS/The Plug AI Progress|Projects]]
- Financial impact: [[../02-PROJECTS/Finance/Financial Goals|Financial goals]]

**[[../Areas/Finance/index|Finance]]** ← Enabled by career + education
- Goal: [[../02-PROJECTS/Finance/Financial Goals|Financial Goals]]
- Requires: Income growth
- Blocked by: Current constraints
- Goal: Independence

**[[../Areas/Education/index|Education]]** ← Enables both career & finance
- Goal: [[../02-PROJECTS/Education/Education Goals|Education Goals]]
- High leverage activity
- Opens doors

**[[../Areas/Development/index|Development]]** ← Supports career goals
- [[../02-PROJECTS/Development/Skills|Skills]]
- [[../02-PROJECTS/Career/Competitive Advantages|Competitive advantages]]

---

## 🚀 Projects Layer

**[[../02-PROJECTS/The Plug AI Progress|The Plug AI]]**
- Builds: [[../02-PROJECTS/Development/Skills|[specific skills]]]
- Supports: [[../02-PROJECTS/Career/Career Goals|Career goals]]
- Contributes to: [[../02-PROJECTS/Finance/Financial Goals|Financial independence]]
- Status: [linked to tracker]

*Additional projects layer in as you create them*

---

## 💡 Opportunities Layer

**[[../Opportunities/index|Opportunities Hub]]**
- Evaluated against: [[../02-PROJECTS/Personal Growth/Core Values|Core Values]]
- Aligned with: [[../02-PROJECTS/Career/Career Goals|Career Goals]] + [[../02-PROJECTS/Finance/Financial Goals|Financial Goals]]
- Leverages: [[../02-PROJECTS/Personal Growth/Strengths|Strengths]]

---

## 📅 Daily Work Layer

**[[../_ops/master-tracker|Master Tracker]]** (Daily focus)
- Shows: Today's priorities
- Linked to: Active projects + goals
- Connects to: Weekly reviews

**[[../_ops/daily-template|Daily Notes]]** (Daily reflection)
- Record: Focus, learning, decisions, reflections
- Link to: Relevant goals + projects
- Feed into: Weekly reviews

---

## 📊 Synthesis Layer

**[[../01-DAILY/Weekly Reviews/index|Weekly Reviews]]** (Pattern recognition)
- Aggregate: Daily insights
- Identify: Patterns across goals
- Surface: Ideas that bubble up
- Inform: Adjustments to goals/vision

**[[../01-DAILY/Progress Dashboard|Progress Dashboard]]** (Bird's-eye view)
- Shows: Status of all areas
- Tracks: Quarterly progress
- Identifies: Cross-area synergies

---

## 🔄 Feedback Loop

```
Daily Work
    ↓
Weekly Insights
    ↓
Goal Adjustments
    ↓
Vision Refinement
    ↓
Back to Daily Work (updated)
```

---

## 🧠 Navigation Patterns

### **Pattern A: Top-Down** (Planning)
1. Start at [[../03-NOTES/Identity/me|Personal Profile]]
2. Review [[../02-PROJECTS/Personal Growth/Core Values|Core Values]] + [[../02-PROJECTS/Personal Growth/Life Vision|Life Vision]]
3. Check progress on [[../02-PROJECTS/Career/Career Goals|Career]], [[../02-PROJECTS/Finance/Financial Goals|Finance]], [[../02-PROJECTS/Education/Education Goals|Education]]
4. Look at [[../02-PROJECTS/The Plug AI Progress|Projects]] advancing goals
5. Decide [[../_ops/daily-template|daily actions]]

### **Pattern B: Bottom-Up** (Execution)
1. Start at [[../_ops/master-tracker|Master Tracker]]
2. Execute [[../02-PROJECTS/The Plug AI Progress|project work]]
3. Reflect in [[../_ops/daily-template|daily notes]]
4. Aggregate in [[../01-DAILY/Weekly Reviews/index|weekly reviews]]
5. Bubble up insights to update [[../02-PROJECTS/Personal Growth/Life Vision|Life Vision]]

### **Pattern C: Network** (Discovery)
1. Open any note
2. Follow related links
3. Discover unexpected connections
4. Refine understanding

---

## 🎯 Connection Rules

**Every note should answer:**
- "Why does this exist?" → Links to bigger purpose
- "What does this enable?" → Links to downstream impact
- "What supports this?" → Links to prerequisites
- "How do I act on this?" → Links to actionable next step

---

## 📍 Hot Spots (Most Connected)

**Most central nodes:**
1. **[[../02-PROJECTS/Personal Growth/Core Values|Core Values]]** ← Everything flows from here
2. **[[../02-PROJECTS/Personal Growth/Life Vision|Life Vision]]** ← Guiding star
3. **[[../02-PROJECTS/Career/Career Goals|Career Goals]]** ← Enables multiple downstream goals
4. **[[../02-PROJECTS/Finance/Financial Goals|Financial Goals]]** ← Critical for independence
5. **[[../01-DAILY/Weekly Reviews/index|Weekly Reviews]]** ← Aggregates all insights

---

## 🔍 Search This Map

**Looking for connections between:**
- Values & Goals? → Check [[../02-PROJECTS/Personal Growth/Life Vision|Life Vision]]
- Goals & Projects? → Check [[../01-DAILY/Progress Dashboard|Progress Dashboard]]
- Daily work & Goals? → Check [[../01-DAILY/Weekly Reviews/index|Weekly Reviews]]
- Skills & Opportunities? → Check [[../Opportunities/index|Opportunities]]
- Everything? → Start at [[00 Home|Home]]

---

## 🛠️ Using This Map

1. **When planning:** Follow arrows downward
2. **When executing:** Follow arrows upward
3. **When lost:** Start at this map
4. **When confused:** Check what node connects to what
5. **When discovering:** Follow links in any direction

---

**This map is your system's operating manual.**

*Last updated: 2026-06-13*
